...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        ghostPositions = [ghost.argMax() for ghost in livingGhostPositionDistributions]
        minDist = float("+inf")
        distance = 0
        bestAction = None
        successorPosition = None

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = min(self.distancer.getDistance(successorPosition, ghostPos) for ghostPos in ghostPositions)
