...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        #given the true distance from Pacman to the ghost. This distribution is modeled by the function
        #busters.getObservationProbability(noisyDistance, trueDistance)
            #which returns P(noisyDistance | trueDistance)
        #You should use this function to help you solve the problem, and use the
        #provided manhattanDistance function to find the distance between Pacman’s location and the ghost’s location.
        # print("HERE", noisyDistance, pacmanPosition, ghostPosition, jailPosition)
        ...
        if noisyDistance == None and ghostPosition == jailPosition:
            ...
            return busters.getObservationProbability(noisyDistance, tDist)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            newProb = self.getObservationProb(observation, pacmanPos, ghostPos, jailPos)
            #Baye's Rule! P( obs|ghost in location ) * P(ghost in location)
            self.beliefs[ghostPos] = newProb * self.beliefs[ghostPos]
    ...
    def elapseTime(self, gameState):
        ...
        newBeliefs = Counter() #standard dict doesn't work due to floats in the coords
        #Mini-forward algorithm: P(newPos) = SUM over oldPos: P(newPos | oldPos) * P(oldPos)
        #
        for oldPos in self.beliefs:
            oldProb = self.beliefs[oldPos]
            newDistribution = self.getPositionDistribution(gameState, oldPos)

            for newPos in newDistribution:
                newProb = newDistribution[newPos]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for partPos in self.particles:
            ...
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(observation[i], pacmanPos, partPos[i], self.getJailPosition(i))
            particleBeliefs[partPos] += prob
